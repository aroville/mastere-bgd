...
MeanAcc_summary = tf.merge_summary([
    tf.scalar_summary('Acc_Train', Acc_Train),
    tf.scalar_summary('Acc_Test', Acc_Test)])
...
for it in xrange(nbIt):
    ...
    if it%1000 == 0:
        ...
        summary_acc = sess.run(
            MeanAcc_summary,
            feed_dict={Acc_Train: Acc_Train_value, Acc_Test: Acc_Test_value})
