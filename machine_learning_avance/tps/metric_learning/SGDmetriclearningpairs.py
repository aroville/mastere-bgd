...
if dataset == 'iris':  # use iris
    iris = datasets.load_iris()
    X = iris.data
    y = iris.target
elif dataset == 'digits':  # use digits
...
def sgd_metric_learning_pairs(X, y, gamma, alpha, n_iter, n_eval, M_ini,
    ...
    for t in range(n_iter):
        ...
        gradient += alpha * np.diag(np.diag(M))
...
e, v = np.linalg.eig(M)
print("Nb de valeurs propres non nulles de M: ", np.sum(e < 1e-12), "/", e.shape[0])
