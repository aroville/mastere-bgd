...
def stochastic_gradient(X, y, gamma, n_iter, w_ini, loss='mse',
    ...
    for t in range(n_iter):
        ...
        if loss == 'mse':
            ...
            pobj[t] = 0.5 * np.mean((y - np.dot(X, w)) ** 2) + 0.5 * alpha * np.linalg.norm(w)**2
        elif loss == 'hinge':
            gradient = (- y[idx] * X[idx] if np.dot(X[idx], w) * y[idx] < 1 else 0) + alpha * w
            pobj[t] = np.mean(np.maximum(0, 1 - y * np.dot(X, w))) + 0.5 * alpha * np.linalg.norm(w)**2
...
#                                          loss=loss, alpha=0.001, average=False)
...
def plot_2d(X, y, w=None):
    """ Plot in 2D the data set, colors and symbols according to the
    class given by the vector y (if given); the separating hyperplane w can
