...
def loss_hinge(x, tau):
    """ value for hinge loss"""
    if isinstance(x, np.ndarray):
        z = np.zeros(x.shape)
        j = x < tau
        z[j] = 1 - x[j]
...
def loss_logistic(x, tau):
    """ value for logistic loss"""
    z = np.log(1 + np.exp(-x)) / np.log(tau)
    return z


def loss_square(x, tau):
    """ value for l_2^2 loss"""
    return (x - tau) ** 2
